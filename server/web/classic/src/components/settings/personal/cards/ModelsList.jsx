...
const ModelsList = ({ t, models, modelsLoading, copyText }) => {
  ...
  const publicModels = (Array.isArray(models) ? models : []).map((model) =>
    typeof model === 'string'
      ? {
          model_name: model,
  ...
  const providerTabs = publicModels.reduce(
    (groups, model) => {
      const key = model.provider_family || 'other';
      if (!groups[key]) {
        groups[key] = {
          label: model.provider_name || t('其他'),
          models: [],
        };
      }
    ...
    { all: { label: t('全部'), models: publicModels } },
  );
          ...
          <>
            ...
            <div className='mb-4'>
              <Tabs
                ...
                collapsible
              >
                {Object.entries(providerTabs).map(([key, category]) => {
                  // 计算该分类下的模型数量
                  const modelCount =
                    key === 'all'
                      ? publicModels.length
              ...
              {(() => {
                ...
                const filteredModels =
                  activeModelCategory === 'all'
                    ? publicModels
                    : providerTabs[activeModelCategory]?.models || [];
                  ...
                  return (
                    <Space wrap>
                      {filteredModels.map((model) =>
                        renderModelTag(model.model_name, {
                          size: 'small',
                          shape: 'circle',
