...
import { getCreatedTokenCredential } from '../../../../helpers/tokenCreation';
...
const EditTokenModal = (props) => {
  ...
  const loadModels = async () => {
    ...
    const { success, message, data, catalog } = res.data;
    if (success) {
      ...
      let localModelOptions = data.map((model) => {
        const publicModel = catalogByName.get(model);
        return {
          label: (
            <span className='flex items-center justify-between gap-2 w-full'>
              <span>{model}</span>
              {publicModel?.provider_name && (
          ...
          value: model,
          providerFamily: publicModel?.provider_family,
        };
      });
  ...
  const submit = async (values) => {
    ...
    if (isEdit) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        const { success, message } = res.data;
        const credential = getCreatedTokenCredential(res, localInputs.name);
        if (success && credential) {
          successCount++;
          credentials.push(credential);
        } else {
          showError(
            t(message || '令牌创建响应未包含密钥，请删除该令牌并重新创建。'),
          );
          break;
        }
      }
  ...
  return (
    <>
      ...
      <Modal
        title={t('保存 API 密钥')}
        visible={createdCredentials.length > 0}
        onOk={() => setCreatedCredentials([])}
        onCancel={() => setCreatedCredentials([])}
        okText={t('我已保存')}
        cancelButtonProps={{ style: { display: 'none' } }}
        closeOnEsc={false}
        maskClosable={false}
      >
        <Text>{t('API 密钥仅显示这一次。关闭前请将其保存在安全的位置。')}</Text>
        <div className='mt-3'>
          {createdCredentials.map((credential) => (
            <div key={credential.id} className='mb-3'>
              <Text strong>{credential.name}</Text>
              <Typography.Paragraph
                code
                copyable={{ content: credential.key }}
