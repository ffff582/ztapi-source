  ...
  return (
    <div>
      ...
      <div className='text-gray-600'>
        ...
        {modelData?.provider_family && (
          <div className='mb-3 flex flex-wrap gap-2'>
            <Tag color='blue'>{modelData.vendor_name}</Tag>
            <Tag color='grey'>
              {modelData.billing_rule === 'multi_dimension'
                ? t('多维度计费')
                : t('按 Token 计费')}
            </Tag>
          </div>
        )}
        {Array.isArray(modelData?.billing_dimensions) &&
          modelData.billing_dimensions.length > 0 && (
            <div className='mb-3 text-sm'>
              {t('计费维度')}：{modelData.billing_dimensions.join(', ')}
            </div>
          )}
        {modelData?.sale_usd && (
          <div className='mb-3'>
            <div className='mb-2 text-sm'>{t('公开售价（美元）')}</div>
            <Space wrap>
              {Object.entries(modelData.sale_usd).map(([dimension, price]) => (
                <Tag key={dimension} color='cyan' size='small'>
                  {dimension}: ${price}
                </Tag>
              ))}
            </Space>
