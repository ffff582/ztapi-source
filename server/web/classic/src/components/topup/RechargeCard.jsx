...
const RechargeCard = ({
  ...
  enableUSDTTRC20TopUp = false,
  usdtTRC20MinTopUp = 10,
  usdtTRC20OrderTTL = 600,
  usdtRequest,
  onUSDTSettled,
}) => {
  ...
  const [usdtOrder, setUSDTOrder] = useState(null);
  ...
  const topupContent = (
    <Space vertical style={{ width: '100%' }}>
      ...
      >
        ...
        {!statusLoading && enableUSDTTRC20TopUp &&
          (usdtOpen ? (
            <USDTTopUpPanel
              enabled
              minimumTopUp={usdtTRC20MinTopUp}
              orderTTLSeconds={usdtTRC20OrderTTL}
              initialOrder={usdtOrder}
              request={usdtRequest}
              onOrderChange={setUSDTOrder}
              onSettled={onUSDTSettled}
              onClose={() => setUSDTOpen(false)}
            />
          ) : (
            <div className='flex flex-col gap-3 border-t border-semi-color-border pt-5 sm:flex-row sm:items-center sm:justify-between'>
              <div className='flex min-w-0 items-center gap-3'>
                <CircleDollarSign size={22} aria-hidden='true' />
                <div>
                  <Text strong>{t('USDT 充值')}</Text>
                  <div>
                    <Text type='tertiary'>
                      {t('TRC-20 网络，最低')} {usdtTRC20MinTopUp} USDT
                    </Text>
                  </div>
                </div>
              </div>
              <Button
                type='primary'
                theme='outline'
                onClick={() => setUSDTOpen(true)}
              >
                {usdtOrder ? t('继续支付') : t('使用 USDT 充值')}
              </Button>
            </div>
          ))}
      </Card>
