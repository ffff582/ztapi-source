...
import {
  ...
  clearAdminSession,
  getAdminSession,
} from '../ztapi/auth/admin-session';
import {
  clearUserSession,
  getUserSession,
  userRefresh,
} from '../ztapi/auth/user-session';

const isZtapiAdminBuild = import.meta.env.VITE_ZTAPI_ADMIN_APP === 'true';
...
export let API = axios.create({
  ...
  withCredentials: isZtapiAdminBuild,
});
...
installAdminRequestInterceptor(API);
installUserRequestInterceptor(API);
...
export function updateAPI() {
  ...
  installAdminRequestInterceptor(API);
  installUserRequestInterceptor(API);
  installAdminResponseInterceptor(API);
}

API.interceptors.response.use(
  (response) =>
    isZtapiAdminBuild ? handleAdminResponseSuccess(API, response) : response,
  async (error) => {
    if (isZtapiAdminBuild) {
      return handleAdminResponseError(API, error);
    }
    const config = error.config;
    if (
      error.response?.status === 401 &&
      config &&
      !config.__ztapiUserRetry &&
      !isAdminAuthRequest(config.url)
    ) {
      try {
        await userRefresh();
        config.__ztapiUserRetry = true;
...
function isAdminAuthRequest(url = '') {
...
function isSessionAuthenticatedManagementRequest(config) {
  const identity = config?.__ztapiSessionIdentity;
  if (!identity || !config || isAdminAuthRequest(config.url)) {
    return false;
  }

  return (
    getHeader(config.headers, 'Authorization') ===
      `Bearer ${identity.accessToken}` &&
    getHeader(config.headers, 'New-API-User') === identity.userId
  );
}

function isLegacyAdminAuthFailure(response) {
  return (
    response.status === 200 &&
    response.data?.success === false &&
    response.data?.code === 'AUTH_REAUTH_REQUIRED' &&
    isSessionAuthenticatedManagementRequest(response.config)
  );
}

function installAdminRequestInterceptor(instance) {
  if (!isZtapiAdminBuild) return;

  instance.interceptors.request.use((config) => {
    if (!config.skipAdminAuth && !isAdminAuthRequest(config.url)) {
      const session = getAdminSession();
      if (session) {
        config.__ztapiSessionIdentity = {
          accessToken: session.access_token,
          userId: String(session.user.id),
        };
        if (typeof config.headers?.set === 'function') {
          config.headers.set('Authorization', `Bearer ${session.access_token}`);
          config.headers.set('New-API-User', String(session.user.id));
        } else {
          config.headers = {
            ...config.headers,
        ...
        delete config.__ztapiSessionIdentity;
      }
    } else {
      delete config.__ztapiSessionIdentity;
    }
...
function installUserRequestInterceptor(instance) {
  if (isZtapiAdminBuild) return;

  instance.interceptors.request.use((config) => {
    const session = getUserSession();
    if (!session || isAdminAuthRequest(config.url)) return config;
    if (typeof config.headers?.set === 'function') {
      config.headers.set('Authorization', `Bearer ${session.access_token}`);
      config.headers.set('New-API-User', String(session.user.id));
    } else {
...
async function handleAdminResponseError(instance, error) {
  const config = error.config;
  const canRetry =
    config &&
    (error.response?.status === 401 || error.isLegacyAdminAuthFailure) &&
    !config.__ztapiRetry &&
    !isAdminAuthRequest(config.url);
  ...
  if (canRetry) {
    ...
    if (session) {
      try {
        const tokenAtRequest = getHeader(config.headers, 'Authorization');
        if (tokenAtRequest === `Bearer ${session.access_token}`) {
          await adminRefresh();
        }
        config.__ztapiRetry = true;
  ...
  if (error.response?.status === 401 || error.isLegacyAdminAuthFailure) {
    clearAdminSession();
  }
  if (!config?.skipErrorHandler) {
    showError(new Error('Administrator request failed.'));
  }
  return Promise.reject(error);
}

function handleAdminResponseSuccess(instance, response) {
  if (!isLegacyAdminAuthFailure(response)) {
    return response;
  }

  return handleAdminResponseError(instance, {
    config: response.config,
    isLegacyAdminAuthFailure: true,
    message: 'Administrator authentication request failed.',
    response,
  });
}

function installAdminResponseInterceptor(instance) {
  if (!isZtapiAdminBuild) return;
  instance.interceptors.response.use(
    (response) => handleAdminResponseSuccess(instance, response),
    (error) => handleAdminResponseError(instance, error),
  );
}
