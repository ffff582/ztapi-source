...
var portableFileMIMETypes = map[string]string{
	".txt":  "text/plain",
	".md":   "text/markdown",
	".csv":  "text/csv",
	".log":  "text/plain",
...
func portableFileMIMEType(fileName string) string {
	extension := strings.ToLower(filepath.Ext(strings.TrimSpace(fileName)))
	if mimeType, ok := portableFileMIMETypes[extension]; ok {
		return mimeType
	}
...
func (m *MediaContent) ToFileSource() types.FileSource {
	...
	case ContentTypeFile:
		...
		mimeType := portableFileMIMEType(file.FileName)
