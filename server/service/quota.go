...
func newAudioQuotaInfo(relayInfo *relaycommon.RelayInfo, modelName string, input, output TokenDetails) QuotaInfo {
...
func PreWssConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.RealtimeUsage) error {
	...
	token, err := model.GetTokenByHash(relayInfo.TokenKeyHash, false)
	if err != nil {
		return err
	}
...
func PreConsumeTokenQuota(relayInfo *relaycommon.RelayInfo, quota int) error {
	...
	return model.DecreaseTokenQuota(relayInfo.TokenId, relayInfo.TokenKeyHash, quota)
}
...
func PostConsumeQuota(relayInfo *relaycommon.RelayInfo, quota int, preConsumedQuota int, sendEmail bool) (err error) {
	...
	if !relayInfo.TokenUnlimited {
		if quota > 0 {
			err = model.DecreaseTokenQuota(relayInfo.TokenId, relayInfo.TokenKeyHash, quota)
		} else {
			err = model.IncreaseTokenQuota(relayInfo.TokenId, relayInfo.TokenKeyHash, -quota)
		}
		if err != nil {
			return err
		}
