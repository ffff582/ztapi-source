...
func TestZTAPIUsageValidationInfoFreezesOnlyReadOnlyInputs(t *testing.T) {
	publication := &relaycommon.ZTAPIPublicationSnapshot{Version: 12}
	usageInfo := &relaycommon.ResponsesUsageInfo{}
	original := &relaycommon.RelayInfo{
		RelayFormat:             types.RelayFormatOpenAI,
		FinalRequestRelayFormat: types.RelayFormatClaude,
		ResponsesUsageInfo:      usageInfo,
		UserId:                  44,
	}

	frozen := ztapiUsageValidationInfo(original, publication)

	require.Same(t, publication, frozen.ZTAPIPublicationSnapshot)
	require.Equal(t, types.RelayFormat(types.RelayFormatClaude), frozen.GetFinalRequestRelayFormat())
	require.Same(t, usageInfo, frozen.ResponsesUsageInfo)
	require.Zero(t, frozen.UserId)

	original.FinalRequestRelayFormat = types.RelayFormatOpenAI
	require.Equal(t, types.RelayFormat(types.RelayFormatClaude), frozen.GetFinalRequestRelayFormat())
}
