...
type Channel struct {
	...
	ZTAPIManaged       bool   `json:"-" gorm:"column:ztapi_managed;not null;default:false;index"`
	ZTAPIFamily        string `json:"-" gorm:"column:ztapi_family;size:32;not null;default:''"`
	ZTAPIKeyCiphertext string `json:"-" gorm:"column:ztapi_key_ciphertext;type:varchar(4096);not null;default:''"`
...
func UpdateChannelStatus(channelId int, usingKey string, status int, reason string) bool {
	...
	if err != nil {
		...
		err = saveChannelStatusAndReconcile(channel, shouldUpdateAbilities)
		if err != nil {
...
func EnableChannelByTag(tag string) error {
	err := withZTAPICatalogWrite(func(tx *gorm.DB) error {
		...
		if err := tx.Model(&Ability{}).Where("tag = ?", tag).Select("enabled").Update("enabled", true).Error; err != nil {
			return err
		}
		_, err := reconcileZTAPIPublishedRoutesTx(tx)
		return err
...
func DisableChannelByTag(tag string) error {
	err := withZTAPICatalogWrite(func(tx *gorm.DB) error {
		...
		if err := tx.Model(&Ability{}).Where("tag = ?", tag).Select("enabled").Update("enabled", false).Error; err != nil {
			return err
		}
		_, err := reconcileZTAPIPublishedRoutesTx(tx)
		return err
...
func EditChannelByTag(tag string, newTag *string, modelMapping *string, models *string, group *string, priority *int64, weight *uint, paramOverride *string, headerOverride *string) error {
	...
	err := withZTAPICatalogWrite(func(tx *gorm.DB) error {
		...
		if shouldReCreateAbilities {
			...
			if err := tx.Model(&Ability{}).Where("tag = ?", tag).Updates(ability).Error; err != nil {
				return err
			}
		}
		_, err := reconcileZTAPIPublishedRoutesTx(tx)
