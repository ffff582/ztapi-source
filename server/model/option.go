...
var (
	ErrOptionValueConflict      = errors.New("option value conflict")
	ErrOptionRuntimeSyncPending = errors.New("option committed but runtime synchronization is pending")
	optionCASSQLiteMu           sync.Mutex
)
...
func optionKeyEquals(key string) map[string]any {
	return map[string]any{"key": key}
}

func optionKeyValueEquals(key, value string) map[string]any {
...
// UpdateOptionIfMatches persists a supported setting only when its stored or
// runtime-default value still equals expectedValue.
func UpdateOptionIfMatches(key, expectedValue, nextValue, runtimeDefault string) error {
	if DB.Dialector.Name() == "sqlite" {
		optionCASSQLiteMu.Lock()
		defer optionCASSQLiteMu.Unlock()
	}
	err := DB.Transaction(func(tx *gorm.DB) error {
		var option Option
		query := tx.Where(optionKeyEquals(key))
		if tx.Dialector.Name() != "sqlite" {
			query = query.Clauses(clause.Locking{Strength: "UPDATE"})
		}
		err := query.Take(&option).Error
		if errors.Is(err, gorm.ErrRecordNotFound) {
			if runtimeDefault != expectedValue {
				return ErrOptionValueConflict
			}
			if err := tx.Create(&Option{Key: key, Value: nextValue}).Error; err != nil {
				return err
			}
			return nil
		}
		...
		result := tx.Model(&Option{}).Where(optionKeyValueEquals(key, expectedValue)).Update("value", nextValue)
		if result.Error != nil {
			return result.Error
		}
	...
	if err != nil {
		if !errors.Is(err, ErrOptionValueConflict) {
			var current Option
			if lookupErr := DB.Where(optionKeyEquals(key)).Take(&current).Error; lookupErr == nil && current.Value != expectedValue {
				return ErrOptionValueConflict
			}
		}
		return err
	}
	if err := updateOptionMap(key, nextValue); err != nil {
		return fmt.Errorf("%w: %v", ErrOptionRuntimeSyncPending, err)
	}
	return nil
}
