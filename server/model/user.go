...
type User struct {
	...
	DebtSuspended    bool           `json:"-" gorm:"not null;default:false;column:debt_suspended"`
...
func SearchUsers(keyword string, group string, role *int, status *int, startIdx int, num int) ([]*User, int64, error) {
	return SearchUsersWithEmailVisibility(keyword, group, role, status, true, startIdx, num)
}

func SearchUsersWithEmailVisibility(keyword string, group string, role *int, status *int, includeEmail bool, startIdx int, num int) ([]*User, int64, error) {
	var users []*User
	var total int64
	var err error
...
func CreateZTAPIUserWithEncodedPassword(username string, encodedPassword string) (*User, error) {
	...
	sensitiveDB := DB.Session(&gorm.Session{
...
func GetZTAPIUserForLogin(username string) (*User, error) {
	var user User
...
// UpdateWithEncodedPassword updates the self-service profile while preserving
// a password hash that was produced by the caller's authentication scheme.
func (user *User) UpdateWithEncodedPassword(encodedPassword string) error {
	...
	return updateUserCache(persisted)
}
...
// EditWithEncodedPassword updates the administrator-editable profile while
// preserving a password hash that was produced by the caller's auth scheme.
func (user *User) EditWithEncodedPassword(encodedPassword string) error {
	updates := map[string]interface{}{
		"username":     user.Username,
		"display_name": user.DisplayName,
		"group":        user.Group,
