...
func (channel *Channel) AddAbilities(tx *gorm.DB) error {
	...
	if err := ValidateZTAPISourceModelNames(useDB, models_); err != nil {
		return err
	}
...
func (channel *Channel) UpdateAbilities(tx *gorm.DB) error {
	if tx == nil {
		err := withZTAPICatalogWrite(func(catalogTx *gorm.DB) error {
			if err := channel.UpdateAbilities(catalogTx); err != nil {
				return err
			}
			_, err := reconcileZTAPIPublishedRoutesTx(catalogTx)
			return err
		})
		if err == nil {
	...
	models_ := strings.Split(channel.Models, ",")
	if err := ValidateZTAPISourceModelNames(tx, models_); err != nil {
		if isNewTx {
			tx.Rollback()
		}
		return err
