...
func GetFullRequestURL(baseURL string, requestURL string, channelType int) string {
	joinBaseURL := baseURL
	joinRequestURL := requestURL
	if channelType == constant.ChannelTypeOpenAI {
		trimmedBaseURL := strings.TrimRight(baseURL, "/")
		if strings.HasSuffix(trimmedBaseURL, "/v1") &&
			(requestURL == "/v1" || strings.HasPrefix(requestURL, "/v1/") || strings.HasPrefix(requestURL, "/v1?")) {
			joinBaseURL = trimmedBaseURL
			joinRequestURL = strings.TrimPrefix(requestURL, "/v1")
		}
	}
	fullRequestURL := fmt.Sprintf("%s%s", joinBaseURL, joinRequestURL)
