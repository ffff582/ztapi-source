...
type RelayInfo struct {
	...
	ReasoningEffortReceived  string
	ReasoningEffortForwarded string
	ReasoningEffortSource    string
	ReasoningTokensReported  *int
...
// billing quantities.
type ZTAPIValidatedImageResponse struct {
...
// ZTAPIImageResponseAttempt privately binds one validated upstream body to one
// adaptor pass. Promotion publishes the billing handoff before client output.
type ZTAPIImageResponseAttempt struct {
	ID              uint64
...
func (info *RelayInfo) BeginZTAPIImageResponseAttempt() uint64 {
	if info == nil {
		return 0
	}
	info.ztapiImageResponseMu.Lock()
	defer info.ztapiImageResponseMu.Unlock()
	info.ztapiImageAttemptSequence++
	if info.ztapiImageAttemptSequence == 0 {
		info.ztapiImageAttemptSequence++
	}
	info.ztapiValidatedImageResponse = nil
	info.ztapiMediaUsageEvidence = nil
	info.ztapiImageResponseAttempt = &ZTAPIImageResponseAttempt{ID: info.ztapiImageAttemptSequence}
	return info.ztapiImageAttemptSequence
}

func cloneZTAPIValidatedImageResponse(source *ZTAPIValidatedImageResponse) *ZTAPIValidatedImageResponse {
	if source == nil {
		return nil
	}
	clone := *source
...
func (info *RelayInfo) RecordZTAPIImageResponseValidation(attemptID uint64, candidate *ZTAPIValidatedImageResponse) bool {
	if info == nil || attemptID == 0 || candidate == nil || len(candidate.RawResponse) == 0 {
		return false
	}
	info.ztapiImageResponseMu.Lock()
	defer info.ztapiImageResponseMu.Unlock()
	if info.ztapiImageResponseAttempt == nil || info.ztapiImageResponseAttempt.ID != attemptID {
		return false
	}
	info.ztapiImageResponseAttempt.BodySHA256 = sha256.Sum256(candidate.RawResponse)
	info.ztapiImageResponseAttempt.ContractVersion = candidate.ContractVersion
	info.ztapiImageResponseAttempt.EvidenceHash = candidate.EvidenceHash
	info.ztapiImageResponseAttempt.Candidate = cloneZTAPIValidatedImageResponse(candidate)
	return true
}

func (info *RelayInfo) CurrentZTAPIImageResponseAttemptID() uint64 {
	if info == nil {
		return 0
	}
	info.ztapiImageResponseMu.Lock()
	defer info.ztapiImageResponseMu.Unlock()
	if info.ztapiImageResponseAttempt == nil {
		return 0
	}
	return info.ztapiImageResponseAttempt.ID
}

func (info *RelayInfo) ztapiImageResponseMatchesCurrentAttemptLocked(body []byte) bool {
	if info.ztapiImageResponseAttempt == nil || info.ztapiImageResponseAttempt.Candidate == nil ||
		info.ZTAPIPublicationSnapshot == nil || info.ZTAPIPublicationSnapshot.ImageProtocolContract == nil {
		return false
	}
	contract := info.ZTAPIPublicationSnapshot.ImageProtocolContract
	return info.ztapiImageResponseAttempt.BodySHA256 == sha256.Sum256(body) &&
		info.ztapiImageResponseAttempt.ContractVersion == contract.Version &&
		info.ztapiImageResponseAttempt.EvidenceHash == contract.EvidenceHash
}

func (info *RelayInfo) ZTAPIImageResponseMatchesCurrentAttempt(body []byte) bool {
	if info == nil {
		return false
	}
	info.ztapiImageResponseMu.Lock()
	defer info.ztapiImageResponseMu.Unlock()
	return info.ztapiImageResponseMatchesCurrentAttemptLocked(body)
}

// CloneCurrentZTAPIImageResponseCandidate returns an attempt-bound copy for
// normalization without publishing the candidate as a billing handoff.
func (info *RelayInfo) CloneCurrentZTAPIImageResponseCandidate(body []byte) *ZTAPIValidatedImageResponse {
	if info == nil {
		return nil
	}
	info.ztapiImageResponseMu.Lock()
	defer info.ztapiImageResponseMu.Unlock()
	if !info.ztapiImageResponseMatchesCurrentAttemptLocked(body) {
		return nil
	}
	return cloneZTAPIValidatedImageResponse(info.ztapiImageResponseAttempt.Candidate)
}

func (info *RelayInfo) clonePublishedZTAPIImageResponse() *ZTAPIValidatedImageResponse {
	if info == nil {
		return nil
	}
	info.ztapiImageResponseMu.Lock()
	defer info.ztapiImageResponseMu.Unlock()
	return cloneZTAPIValidatedImageResponse(info.ztapiValidatedImageResponse)
}

// PromoteZTAPIValidatedImageResponse is the only transition from private
// attempt evidence to the public billing handoff. It verifies and publishes
// the response and usage evidence under one lock before client output.
func (info *RelayInfo) PromoteZTAPIValidatedImageResponse(body []byte, evidence ZTAPIMediaUsageEvidence) bool {
	if info == nil {
		return false
	}
	info.ztapiImageResponseMu.Lock()
	...
	candidate := info.ztapiImageResponseAttempt.Candidate
	if !evidence.ResultAvailable || evidence.UpstreamRequestID != candidate.UpstreamRequestID ||
		evidence.ResultCount != candidate.ResultCount || !bytes.Equal(evidence.GetRawUsageJSON(), candidate.RawUsageJSON) ||
		evidence.Pending != (evidence.Reason != "") {
		return false
	}
	frozenEvidence := evidence.Clone()
	info.ztapiMediaUsageEvidence = &frozenEvidence
	info.ztapiValidatedImageResponse = cloneZTAPIValidatedImageResponse(candidate)
	info.ztapiImageResponseAttempt = nil
	return true
}

// GetZTAPIImageSettlementEvidence returns one generation of response and usage
// evidence under the same lock. Callers must not combine separate snapshots.
func (info *RelayInfo) GetZTAPIImageSettlementEvidence() (*ZTAPIValidatedImageResponse, *ZTAPIMediaUsageEvidence) {
	if info == nil {
		return nil, nil
	}
	info.ztapiImageResponseMu.Lock()
	defer info.ztapiImageResponseMu.Unlock()
	if info.ztapiValidatedImageResponse == nil || info.ztapiMediaUsageEvidence == nil {
		return nil, nil
	}
	usage := info.ztapiMediaUsageEvidence.Clone()
	return cloneZTAPIValidatedImageResponse(info.ztapiValidatedImageResponse), &usage
}

func (info *RelayInfo) GetZTAPIMediaUsageEvidence() *ZTAPIMediaUsageEvidence {
	_, usage := info.GetZTAPIImageSettlementEvidence()
	return usage
}

func (info *RelayInfo) AbortZTAPIImageResponseAttempt(attemptID uint64) {
	if info == nil || attemptID == 0 {
		return
	}
	info.ztapiImageResponseMu.Lock()
	defer info.ztapiImageResponseMu.Unlock()
...
func (info *RelayInfo) ToString() string {
	...
	fmt.Fprintf(b, "SelectionModelName: %q, ", info.SelectionModelName)
...
func genBaseRelayInfo(c *gin.Context, request dto.Request) *RelayInfo {
	...
	info := &RelayInfo{
		...
		SelectionModelName:       common.GetContextKeyString(c, constant.ContextKeySelectionModel),
		ZTAPIPublicationSnapshot: GetZTAPIPublicationSnapshot(c),

		TokenId:        common.GetContextKeyInt(c, constant.ContextKeyTokenId),
		TokenKeyHash:   common.GetContextKeyString(c, constant.ContextKeyTokenKeyHash),
	...
	CaptureReasoningEffortReceived(info, request)
...
const ztapiTaskProviderAcceptedContextKey = "ztapi_task_provider_accepted"

func MarkZTAPITaskProviderAccepted(c *gin.Context) {
	if c != nil {
		c.Set(ztapiTaskProviderAcceptedContextKey, true)
	}
}

func ZTAPITaskProviderAccepted(c *gin.Context) bool {
	return c != nil && c.GetBool(ztapiTaskProviderAcceptedContextKey)
}
...
type TaskInfo struct {
	...
	ProviderStatus    string            `json:"-"`
	UpstreamRequestID string            `json:"-"`
	UpstreamTaskID    string            `json:"-"`
	UsageDimensions   map[string]string `json:"-"`
	ResultMetadata    map[string]string `json:"-"`
}
