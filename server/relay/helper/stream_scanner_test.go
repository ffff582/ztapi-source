...
func TestStreamingTimeoutDurationUsesSafeDefault(t *testing.T) {
	assert.Equal(t, 300*time.Second, streamingTimeoutDuration(0))
	assert.Equal(t, 300*time.Second, streamingTimeoutDuration(-1))
	assert.Equal(t, 2*time.Second, streamingTimeoutDuration(2))
}
...
func TestStreamScannerHandler_ScannerDecoupledFromSlowHandler(t *testing.T) {
	...
	c, resp, info := setupStreamTest(t, pipeReader)
	handlerStarted := make(chan struct{})
	releaseHandler := make(chan struct{})
	var startOnce sync.Once
	var releaseOnce sync.Once
	unblockHandler := func() { releaseOnce.Do(func() { close(releaseHandler) }) }
	...
	go func() {
		for i := 0; i < numChunks; i++ {
			if _, err := fmt.Fprintf(pipeWriter, "data: {\"id\":%d}\n", i); err != nil {
				writerDone <- err
				return
			}
		}
		writerDone <- pipeWriter.Close()
	}()
	...
	case <-time.After(2 * time.Second):
		t.Fatal("upstream remained blocked even though the burst fits in the scanner dispatch buffer")
	}
	assert.Zero(t, count.Load(), "handler should still be blocked while scanner drains upstream")
	unblockHandler()
	select {
	case <-done:
	case <-time.After(2 * time.Second):
