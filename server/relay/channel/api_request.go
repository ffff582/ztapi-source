...
func DoWssRequest(a Adaptor, c *gin.Context, info *common.RelayInfo, requestBody io.Reader) (*websocket.Conn, error) {
	...
	healthAttempt.Dispatched()
	targetConn, _, err := websocket.DefaultDialer.Dial(fullRequestURL, targetHeader)
	if err != nil {
		healthAttempt.TransportError(err)
...
func doRequest(c *gin.Context, req *http.Request, info *common.RelayInfo) (*http.Response, error) {
	...
	if info != nil && info.ZTAPIPublicationSnapshot != nil && info.ZTAPIPublicationSnapshot.Modality == "image" {
		// Managed image contracts freeze the exact upstream path. Clone the
		// selected client so shared and proxy clients retain legacy behavior.
		managedClient := *client
		managedClient.CheckRedirect = func(*http.Request, []*http.Request) error {
	...
	if err := service.BeginZTAPIBillingAttempt(info, req.URL.Path); err != nil {
	...
	if resp == nil {
		healthAttempt.TransportError(errors.New("nil upstream response"))
		return nil, errors.New("resp is nil")
	}
	healthAttempt.WrapResponse(resp)
	if err := service.ObserveZTAPIBillingResponse(info, resp); err != nil {
		_ = resp.Body.Close()
		return nil, types.NewError(err, types.ErrorCodeDoRequestFailed, types.ErrOptionWithSkipRetry())
	}
