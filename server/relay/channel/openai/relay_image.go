...
func OpenaiImageHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (usage *dto.Usage, apiErr *types.NewAPIError) {
	defer service.CloseResponseBodyGracefully(resp)
	managedImage := info != nil && info.ZTAPIPublicationSnapshot != nil && info.ZTAPIPublicationSnapshot.Modality == "image"
	attemptID := uint64(0)
	adaptorSucceeded := false
	if managedImage {
	...
	if managedImage && attemptID == 0 {
		return nil, types.NewErrorWithStatusCode(fmt.Errorf("managed image response was not validated against its frozen contract"), types.ErrorCodeBadResponseBody, http.StatusInternalServerError, types.ErrOptionWithSkipRetry())
	}
	...
	if managedImage && !info.ZTAPIImageResponseMatchesCurrentAttempt(responseBody) {
		return nil, types.NewErrorWithStatusCode(fmt.Errorf("managed image response does not match the current validated attempt"), types.ErrorCodeBadResponseBody, http.StatusBadGateway, types.ErrOptionWithSkipRetry())
	}
	...
	if managedImage {
		...
		if err := common.Unmarshal(responseBody, &errorEnvelope); err != nil {
		...
		if oaiError := dto.GetOpenAIError(errorEnvelope.Error); oaiError != nil && oaiError.Type != "" {
			return nil, types.WithOpenAIError(*oaiError, resp.StatusCode)
		}
		candidate := info.CloneCurrentZTAPIImageResponseCandidate(responseBody)
		if candidate == nil {
		...
		if !info.PromoteZTAPIValidatedImageResponse(responseBody, evidence) {
			return nil, types.NewErrorWithStatusCode(fmt.Errorf("managed image adaptor could not accept the current validated attempt"), types.ErrorCodeBadResponseBody, http.StatusBadGateway, types.ErrOptionWithSkipRetry())
		}
		adaptorSucceeded = true
