...
func TestRequestOpenAI2ClaudeMessage_RejectsUnsupportedFileContent(t *testing.T) {
				...
				Content: []any{
					...
					dto.MediaContent{
						...
						File: &dto.MessageFile{
							FileName: "blob.bin",
							FileData: base64.StdEncoding.EncodeToString([]byte{0, 1, 2, 3}),
						},
					},
				},
			},
	...
	require.Error(t, err)
	require.Contains(t, err.Error(), "unsupported file MIME type")
}
...
func TestRequestOpenAI2ClaudeMessage_ConvertsMarkdownFileWithoutOSMimeDatabase(t *testing.T) {
	request := dto.GeneralOpenAIRequest{
		Model: "claude-3-5-sonnet",
		Messages: []dto.Message{{
			Role: "user",
			Content: []any{dto.MediaContent{
				...
				File: &dto.MessageFile{
					...
					FileData: base64.StdEncoding.EncodeToString([]byte("# Release\n\nSafe rollout")),
				},
			}},
		}},
	}
