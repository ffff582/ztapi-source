			...
			if message.Role == "tool" {
				...
				for _, mediaMessage := range message.ParseContent() {
					...
					default:
						...
						if strings.HasPrefix(mimeType, "application/pdf") {
						...
						} else if isClaudeTextAttachmentMIME(mimeType) {
							textData, decodeErr := base64.StdEncoding.DecodeString(base64Data)
							if decodeErr != nil {
								return nil, fmt.Errorf("decode text file for Claude: %w", decodeErr)
							}
							...
							return nil, fmt.Errorf("unsupported file MIME type %q for Claude", mimeType)
						}
...
func isClaudeTextAttachmentMIME(mimeType string) bool {
