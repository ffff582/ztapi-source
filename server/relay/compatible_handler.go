...
func TextHelper(c *gin.Context, info *relaycommon.RelayInfo) (newAPIError *types.NewAPIError) {
	...
	if shouldUseAudioQuota(info, usage.(*dto.Usage)) {
...
func shouldUseAudioQuota(info *relaycommon.RelayInfo, usage *dto.Usage) bool {
	if info == nil || usage == nil {
		return false
	}
	containsAudioTokens := usage.CompletionTokenDetails.AudioTokens > 0 || usage.PromptTokensDetails.AudioTokens > 0
	if !containsAudioTokens {
		return false
	}
	if info.ZTAPIPublicationSnapshot != nil {
		return true
	}
	return ratio_setting.ContainsAudioRatio(info.OriginModelName) ||
