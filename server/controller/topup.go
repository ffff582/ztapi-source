...
func GetTopUpInfo(c *gin.Context) {
	...
	usdtConfig, usdtConfigErr := setting.LoadUSDTTopUpConfig()
	enableUSDTTRC20 := complianceConfirmed && usdtConfigErr == nil && usdtConfig.Enabled
	...
	data := gin.H{
		...
		"enable_usdt_trc20_topup":          enableUSDTTRC20,
		"usdt_trc20_network":               model.USDTTopUpNetworkTronMainnet,
		"usdt_trc20_asset":                 model.USDTTopUpAssetUSDT,
		"usdt_trc20_min_topup":             usdtConfig.MinTopUp,
		"usdt_trc20_order_ttl_seconds":     int64(usdtConfig.OrderTTL / time.Second),
...
func AdminCompleteTopUp(c *gin.Context) {
	...
	if req.ExpectedStatus != common.TopUpStatusPending || req.Reason == "" || len(req.Reason) > 500 {
		c.JSON(http.StatusBadRequest, gin.H{"success": false, "message": "reason and expected_status=pending are required"})
		return
	}
	topUp := model.GetTopUpByTradeNo(req.TradeNo)
	if topUp == nil {
		c.JSON(http.StatusNotFound, gin.H{"success": false, "message": "topup not found"})
		return
	}
	applyAdminTopUpMutation(c, topUp.Id, adminTopUpMutationRequest{ExpectedStatus: req.ExpectedStatus, Reason: req.Reason}, common.TopUpStatusSuccess, "topup.complete")
}
