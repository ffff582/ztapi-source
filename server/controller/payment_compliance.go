...
func ConfirmPaymentCompliance(c *gin.Context) {
	if c.GetBool("use_access_token") && !common.GetContextKeyBool(c, constant.ContextKeyZTAPIJWTAuthenticated) {
		c.JSON(http.StatusForbidden, gin.H{
			"success": false,
			"message": "This operation requires a signed dashboard login. Static API access tokens are not allowed.",
		})
		return
	}
