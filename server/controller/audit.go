...
var auditContentTemplates = map[string]string{
	...
	"balance.adjustment":    "Adjusted user ${target_user_id} balance by ${delta}: ${reason} (ledger ${ledger_id}, ${result})",
	"topup.complete":        "Completed topup ${trade_no} for user ${target_user_id}: ${reason}",
	"topup.reject":          "Rejected topup ${trade_no} for user ${target_user_id}: ${reason}",
	"settings.update":       "Updated ZTAPI setting ${key}: ${reason}",
	"export.orders":         "Exported ${row_count} finance orders",
	"export.ledger":         "Exported ${row_count} balance ledger entries",
	"export.request_logs":   "Exported ${row_count} request logs",
	"export.audit_logs":     "Exported ${row_count} audit logs",
	...
	"ztapi.model_below_cost_override": "Confirmed below-cost ZTAPI model ${public_name} (ID: ${id}, version ${version})",
...
func recordManageAuditFor(c *gin.Context, logUserId int, action string, params map[string]interface{}) {
	model.RecordOperationAuditLogWithRequestID(logUserId, auditContentEN(action, params), c.ClientIP(), c.GetString(common.RequestIdKey), action, params, auditOperatorInfo(c), nil)
	markAuditLogged(c)
}
...
func recordUserSecurityAudit(c *gin.Context, userId int, action string, params map[string]interface{}) {
	model.RecordOperationAuditLogWithRequestID(userId, auditContentEN(action, params), c.ClientIP(), c.GetString(common.RequestIdKey), action, params, nil, nil)
}
