...
const ztapiChannelCredentialMask = "********"

var ztapiSupportedChannelTypes = []int{
	constant.ChannelTypeOpenAI,
	constant.ChannelTypeAnthropic,
	constant.ChannelTypeGemini,
}

var ztapiUpstreamURLProtection = &common.SSRFProtection{
...
type ztapiChannelInfoProjection struct {
...
type ztapiChannelProjection struct {
	ID           int                        `json:"id"`
	Type         int                        `json:"type"`
	Key          string                     `json:"key"`
	...
	ChannelInfo  ztapiChannelInfoProjection `json:"channel_info"`
}

func projectZTAPIChannel(channel *model.Channel) ztapiChannelProjection {
	return ztapiChannelProjection{
		ID:           channel.Id,
		Type:         channel.Type,
		Key:          ztapiChannelCredentialMask,
		Status:       channel.Status,
		Name:         channel.Name,
		ResponseTime: channel.ResponseTime,
		BaseURL:      sanitizeZTAPIChannelBaseURL(channel.BaseURL),
		Models:       channel.Models,
		Group:        channel.Group,
		Priority:     channel.GetPriority(),
		Weight:       channel.GetWeight(),
		ModelMapping: channel.GetModelMapping(),
		ChannelInfo: ztapiChannelInfoProjection{
...
func sanitizeZTAPIChannelBaseURL(raw *string) *string {
...
func ztapiChannelQuery() *gorm.DB {
	return model.DB.Model(&model.Channel{}).
		Where("type IN ?", ztapiSupportedChannelTypes).
		Omit("key", "header_override", "param_override", "other", "other_info", "setting", "settings")
}

func GetZTAPIChannels(c *gin.Context) {
	pageInfo := common.GetPageQuery(c)
	query := ztapiChannelQuery()
	var total int64
	if err := query.Count(&total).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"success": false, "message": "无法加载上游渠道。"})
		return
	}
	channels := make([]*model.Channel, 0)
	if err := query.Order("id DESC").Limit(pageInfo.GetPageSize()).Offset(pageInfo.GetStartIdx()).Find(&channels).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"success": false, "message": "无法加载上游渠道。"})
		return
	}
	items := make([]ztapiChannelProjection, 0, len(channels))
	for _, channel := range channels {
		items = append(items, projectZTAPIChannel(channel))
	}
...
func SearchZTAPIChannels(c *gin.Context) {
	pageInfo := common.GetPageQuery(c)
	keyword := strings.TrimSpace(c.Query("keyword"))
	query := ztapiChannelQuery()
	if keyword != "" {
		like := "%" + keyword + "%"
		query = query.Where("name LIKE ? OR models LIKE ? OR base_url LIKE ?", like, like, like)
	}
	var total int64
	if err := query.Count(&total).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"success": false, "message": "无法搜索上游渠道。"})
		return
	}
	channels := make([]*model.Channel, 0)
	if err := query.Order("id DESC").Limit(pageInfo.GetPageSize()).Offset(pageInfo.GetStartIdx()).Find(&channels).Error; err != nil {
...
func GetZTAPIChannel(c *gin.Context) {
	...
	if err := ztapiChannelQuery().Where("id = ?", id).First(&channel).Error; err != nil {
	...
	common.ApiSuccess(c, projectZTAPIChannel(&channel))
}

var fetchZTAPIChannelUpstreamModelIDs = fetchChannelUpstreamModelIDs

func FetchZTAPIUpstreamModels(c *gin.Context) {
	id, err := strconv.Atoi(c.Param("id"))
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"success": false, "message": "渠道 ID 无效。"})
		return
	...
	if err != nil || !channel.ZTAPIManaged || !isZTAPISupportedChannelType(channel.Type) {
		c.JSON(http.StatusNotFound, gin.H{"success": false, "message": "上游渠道不存在。"})
		return
	}
	key, _, keyErr := channel.GetNextEnabledKey()
	if keyErr != nil || strings.TrimSpace(key) == "" {
		c.JSON(http.StatusBadRequest, gin.H{"success": false, "message": "上游渠道凭据不可用。"})
		return
	}
	ids, err := fetchZTAPIChannelUpstreamModelIDs(channel)
	if err != nil {
	...
	if c.Query("import") == "true" || c.Query("import") == "1" {
		result, importErr := model.ImportZTAPIDiscovery(channel.Id, ids, time.Now().UTC())
		if importErr != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"success": false, "message": "无法保存上游模型发现结果。"})
			return
		}
...
func ztapiFamilyForChannelType(channelType int) string {
	switch channelType {
	case constant.ChannelTypeOpenAI:
		return model.ZTAPIModelFamilyOpenAI
	case constant.ChannelTypeAnthropic:
		return model.ZTAPIModelFamilyClaude
	case constant.ChannelTypeGemini:
		return model.ZTAPIModelFamilyGemini
	default:
		return ""
	}
}

type ztapiChannelWriteRequest struct {
	Name         string  `json:"name"`
	Type         int     `json:"type"`
	Key          string  `json:"key"`
	Status       int     `json:"status"`
...
func normalizeZTAPIChannelWriteRequest(request *ztapiChannelWriteRequest, isCreate bool) error {
	request.Name = strings.TrimSpace(request.Name)
	request.Key = strings.TrimSpace(request.Key)
	request.BaseURL = strings.TrimSpace(request.BaseURL)
	request.Models = strings.TrimSpace(request.Models)
	request.Group = strings.TrimSpace(request.Group)
	if request.ModelMapping != nil {
		mapping := strings.TrimSpace(*request.ModelMapping)
	...
	if request.Name == "" || request.Models == "" || request.BaseURL == "" {
		return fmt.Errorf("missing required channel fields")
	}
	if isCreate && request.Status == common.ChannelStatusEnabled && request.Key == "" {
		return fmt.Errorf("missing channel key")
	}
	if !isZTAPISupportedChannelType(request.Type) {
	...
	if request.Priority != nil && *request.Priority < 0 {
		return fmt.Errorf("invalid channel priority")
	}
	if request.Weight != nil && (*request.Weight == 0 || *request.Weight > 1000) {
		return fmt.Errorf("invalid channel weight")
	}
	for _, modelName := range strings.Split(request.Models, ",") {
		if len(strings.TrimSpace(modelName)) > 255 {
			return fmt.Errorf("model name is too long")
		}
	}
	if request.ModelMapping != nil && *request.ModelMapping != "" {
		var mapping map[string]string
		if err := common.Unmarshal([]byte(*request.ModelMapping), &mapping); err != nil {
	...
	if err := validateZTAPIUpstreamBaseURL(request.BaseURL); err != nil {
		return fmt.Errorf("invalid channel base URL")
	}
	return nil
}

func validateZTAPIUpstreamBaseURL(baseURL string) error {
	parsed, err := url.Parse(baseURL)
	if err != nil || parsed.Scheme != "https" || parsed.Host == "" || parsed.User != nil {
		return fmt.Errorf("invalid upstream URL")
	}
	if err := ztapiUpstreamURLProtection.ValidateURL(baseURL); err != nil {
		return fmt.Errorf("unsafe upstream URL: %w", err)
	}
	return nil
}

func ztapiChannelFromRequest(request ztapiChannelWriteRequest) model.Channel {
	...
	return model.Channel{
		...
		AutoBan:      &autoBan,
		ZTAPIManaged: true,
		ZTAPIFamily:  ztapiFamilyForChannelType(request.Type),
	}
}

func AddZTAPIChannel(c *gin.Context) {
	var request ztapiChannelWriteRequest
	if err := c.ShouldBindJSON(&request); err != nil || normalizeZTAPIChannelWriteRequest(&request, true) != nil {
		c.JSON(http.StatusBadRequest, gin.H{"success": false, "message": "上游渠道配置无效。"})
		return
	}
	channel := ztapiChannelFromRequest(request)
	if err := validateChannel(&channel, request.Key != ""); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"success": false, "message": "上游渠道配置无效。"})
		return
	}
	if err := channel.Insert(); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"success": false, "message": "无法创建上游渠道。"})
		return
	}
	model.InitChannelCache()
	service.ResetProxyClientCache()
	recordManageAudit(c, "channel.create", map[string]interface{}{
		"id": channel.Id, "name": channel.Name, "type": channel.Type, "source": "ztapi",
	})
	common.ApiSuccess(c, projectZTAPIChannel(&channel))
}

func UpdateZTAPIChannel(c *gin.Context) {
	id, err := strconv.Atoi(c.Param("id"))
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"success": false, "message": "渠道 ID 无效。"})
		return
	...
	var request ztapiChannelWriteRequest
	if err := c.ShouldBindJSON(&request); err != nil || normalizeZTAPIChannelWriteRequest(&request, false) != nil {
		c.JSON(http.StatusBadRequest, gin.H{"success": false, "message": "上游渠道配置无效。"})
		return
	}
	original, err := model.GetChannelById(id, true)
	if err != nil || !isZTAPISupportedChannelType(original.Type) {
		c.JSON(http.StatusNotFound, gin.H{"success": false, "message": "上游渠道不存在。"})
		return
	}
	baseURL := request.BaseURL
	...
	original.ZTAPIManaged = true
	original.ZTAPIFamily = ztapiFamilyForChannelType(request.Type)
	original.Status = request.Status
	original.BaseURL = &baseURL
	original.Models = request.Models
	...
	if request.ModelMapping != nil {
		original.ModelMapping = request.ModelMapping
	}
	if request.Key != "" {
		original.Key = request.Key
	}
	if original.Status == common.ChannelStatusEnabled && original.Key == "" && original.ZTAPIKeyCiphertext == "" {
		c.JSON(http.StatusBadRequest, gin.H{"success": false, "message": "启用渠道前必须配置 API 密钥。"})
		return
	}
	if err := validateChannel(original, false); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"success": false, "message": "上游渠道配置无效。"})
		return
	}
	if err := original.Update(); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"success": false, "message": "无法保存上游渠道。"})
		return
	}
	model.InitChannelCache()
	service.ResetProxyClientCache()
	recordManageAudit(c, "channel.update", map[string]interface{}{
		"id": original.Id, "name": original.Name, "type": original.Type, "source": "ztapi",
	})
	common.ApiSuccess(c, projectZTAPIChannel(original))
}

func UpdateZTAPIChannelStatus(c *gin.Context) {
	id, err := strconv.Atoi(c.Param("id"))
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"success": false, "message": "渠道 ID 无效。"})
		return
	...
	if err := c.ShouldBindJSON(&request); err != nil || (request.Status != common.ChannelStatusEnabled && request.Status != common.ChannelStatusManuallyDisabled) {
		c.JSON(http.StatusBadRequest, gin.H{"success": false, "message": "渠道状态无效。"})
		return
	}
	channel, err := model.GetChannelById(id, true)
	...
	if request.Status == common.ChannelStatusEnabled && channel.Key == "" && channel.ZTAPIKeyCiphertext == "" {
		c.JSON(http.StatusBadRequest, gin.H{"success": false, "message": "启用渠道前必须配置 API 密钥。"})
		return
	}
	if channel.Status != request.Status && !model.UpdateChannelStatus(id, "", request.Status, "ZTAPI admin status change") {
		c.JSON(http.StatusInternalServerError, gin.H{"success": false, "message": "无法修改渠道状态。"})
		return
	}
	channel, err = model.GetChannelById(id, true)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"success": false, "message": "无法重新加载渠道状态。"})
		return
	}
	model.InitChannelCache()
	recordManageAudit(c, "channel.status_update", map[string]interface{}{
		"id": channel.Id, "name": channel.Name, "status": channel.Status, "source": "ztapi",
	})
	common.ApiSuccess(c, projectZTAPIChannel(channel))
}
...
type KeyStatus struct {
	...
	KeyPreview   string `json:"key_preview"` // fixed mask; never includes credential content
}
...
func ManageMultiKeys(c *gin.Context) {
	...
	if request.Action != "get_key_status" && !common.HasAdminPermission(c.GetInt("role"), common.PermissionChannelWrite) {
		c.JSON(http.StatusForbidden, gin.H{"success": false, "message": "无权修改上游渠道。"})
		return
	}
	...
	case "get_key_status":
		...
		for i := range keys {
			...
			allKeyStatusList = append(allKeyStatusList, KeyStatus{
				...
				KeyPreview:   ztapiChannelCredentialMask,
			})
		}
