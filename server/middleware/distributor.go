...
func ztapiSnapshotGroupAllowed(groups []string, group string) bool {
...
func loadZTAPIPublicationSnapshot(publicName, sourceModel, group string) (*relaycommon.ZTAPIPublicationSnapshot, error) {
	publication, err := model.GetZTAPIRuntimePublication(publicName)
	if err != nil {
		return nil, err
	}
	if publication.SourceModel != sourceModel {
		return nil, errors.New("ZTAPI publication changed during authorization")
	}
	groups := publication.Groups
	if !ztapiSnapshotGroupAllowed(groups, group) {
		return nil, model.ErrZTAPIModelGroupForbidden
	}
	return ztapiPublicationSnapshotFromRuntime(*publication, group), nil
}

func ztapiPublicationSnapshotFromRuntime(publication model.ZTAPIRuntimePublication, group string) *relaycommon.ZTAPIPublicationSnapshot {
	return (&relaycommon.ZTAPIPublicationSnapshot{
		PublicationID:          publication.ModelConfigID,
		...
		CacheCreation5mRatio:   publication.CacheCreation5mRatio,
		CacheCreation1hRatio:   publication.CacheCreation1hRatio,
		ImageRatio:             publication.ImageRatio,
		AudioRatio:             publication.AudioRatio,
		AudioCompletionRatio:   publication.AudioCompletionRatio,
		ImageProtocolContract:  publication.ImageProtocolContract,
		VideoProtocolContract:  publication.VideoProtocolContract,
	}).Clone()
}

func enforceTokenModelAccess(c *gin.Context, requestedModel string) bool {
	if !common.GetContextKeyBool(c, constant.ContextKeyTokenModelLimitEnabled) {
		return true
	}
	publicMatchingModel := strings.TrimSpace(requestedModel)
	if publicMatchingModel == "" {
		abortWithOpenAiMessage(c, http.StatusBadRequest, i18n.T(c, i18n.MsgDistributorModelNameRequired))
		return false
	}

	_, publicAlias := model.ResolveZTAPIPublicAlias(requestedModel)
	if !publicAlias {
	...
	_, publicAllowed := tokenModelLimit[publicMatchingModel]
	if !publicAllowed {
		abortWithOpenAiMessage(c, http.StatusForbidden, i18n.T(c, i18n.MsgDistributorTokenModelForbidden, map[string]any{"Model": requestedModel}))
		return false
	}
...
func Distribute() func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		if requestedModel != "" {
			...
			if errors.Is(resolveErr, model.ErrZTAPIModelNotPublic) {
				abortWithOpenAiMessage(c, http.StatusNotFound, "模型未发布或不可用", types.ErrorCodeModelNotFound)
				return
			}
			if resolveErr != nil {
			...
			publicationGroups = publicationSnapshot.AllowedGroups
			if usingGroup == "auto" && len(publicationGroups) > 0 {
				userGroup := common.GetContextKeyString(c, constant.ContextKeyUserGroup)
				for _, candidate := range service.GetUserAutoGroup(userGroup) {
					if slices.Contains(publicationGroups, "all") || slices.Contains(publicationGroups, candidate) {
						usingGroup = candidate
						common.SetContextKey(c, constant.ContextKeyUsingGroup, candidate)
						common.SetContextKey(c, constant.ContextKeyAutoGroup, candidate)
						break
					}
				}
				if usingGroup == "auto" {
					abortWithOpenAiMessage(c, http.StatusForbidden, "该模型未向当前用户的自动分组开放")
					return
				}
			}
			if publicationSnapshot != nil {
				publicationSnapshot.AuthorizedGroup = usingGroup
				relaycommon.SetZTAPIPublicationSnapshot(c, publicationSnapshot)
			}
		}
		matchingModel := ratio_setting.FormatMatchingModelName(selectionRequestedModel)
		selectionModel := matchingModel
		if selectionModel == "" {
		...
		if ok {
			...
			if publicationSnapshot != nil && !slices.Contains(publicationSnapshot.AllowedChannelIDs, channel.Id) {
				abortWithOpenAiMessage(c, http.StatusForbidden, "specific channel is not authorized for this published model")
				return
			}
			if publicationSnapshot != nil && !model.IsChannelEnabledForGroupModel(usingGroup, selectionModel, channel.Id) {
				abortWithOpenAiMessage(c, http.StatusForbidden, "specific channel is not authorized for the current model group")
				return
			}
		} else {
			// Select a channel for the user. Token model access was checked against
			// the requested public name before publication resolution.
			...
			if shouldSelectChannel {
				...
				if preferredChannelID, found := service.GetPreferredChannelByAffinity(c, selectionModel, usingGroup); found {
					...
					if err == nil && preferred != nil && preferred.Status == common.ChannelStatusEnabled &&
						(publicationSnapshot == nil || slices.Contains(publicationSnapshot.AllowedChannelIDs, preferred.Id)) {
						if usingGroup == "auto" {
							userGroup := common.GetContextKeyString(c, constant.ContextKeyUserGroup)
				...
				if channel == nil {
					channel, selectGroup, err = service.CacheGetRandomSatisfiedChannel(&service.RetryParam{
						...
						AllowedGroups: publicationGroups,
						AllowedChannelIDs: func() []int {
							if publicationSnapshot == nil {
								return nil
							}
							return publicationSnapshot.AllowedChannelIDs
						}(),
...
func SetupContextForSelectedChannel(c *gin.Context, channel *model.Channel, modelName string) *types.NewAPIError {
	...
	if err := validateZTAPIRequestProtocol(c, channel, modelName); err != nil {
		return err
	}
