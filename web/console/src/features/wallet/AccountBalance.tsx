...
// Balances are held in U, the currency customers top up in; a dollar sign
// would name a different currency at a different rate.
export function formatAccountUnits(amount: number) {
...
export function AccountBalance({ refreshKey = '' }: { refreshKey?: string }) {
  ...
  return (
    <section className="account-balance" aria-label={t('账户余额')}>
      ...
      <div className="account-balance__value" aria-live="polite" aria-busy={balance.status === 'loading'}>
        <h2>{t('可用余额')} <span>U</span></h2>
        {balance.status === 'ready' && <strong>{formatAccountUnits(balance.data)}</strong>}
        {balance.status === 'loading' && <p>{t('正在读取余额...')}</p>}
        {balance.status === 'error' && <p role="status">{t('余额加载失败，请刷新重试。')}</p>}
      </div>
      <button className="console-icon-action" aria-label={t('刷新余额')} title={t('刷新余额')}
