...
test('reasoning cases cover declared levels, exact ultra mapping, and pending capability discovery', () => {
  assert.deepEqual(buildReasoningCases([
    ...
    { published: true, modality: 'text', public_name: 'zt-pending', source_model: 'pending', reasoning_efforts: [], live_validation_required: true },
...
test('pending discovery accepts only audited success or an explicit effort rejection', async () => {
  const fetchImpl = async (url, options = {}) => {
    if (url.includes('/api/models/ztapi/')) {
      return new Response(JSON.stringify({ success: true, data: { items: [{ published: true, modality: 'text', public_name: 'zt-pending', source_model: 'pending', reasoning_efforts: [], live_validation_required: true }] } }), { status: 200 });
    }
    ...
    if (effort === 'none') return new Response(JSON.stringify({ id: 'accepted', status: 'completed' }), { status: 200 });
...
test('matrix stops before budget ceiling and sends neutral 64-token requests', async () => {
  ...
  const fetchImpl = async (url, options = {}) => {
    ...
    if (url.includes('/api/admin/request-logs')) {
	  const requestId = new URL(url).searchParams.get('request_id');
	  const request = outbound.find((entry) => entry.url.endsWith('/v1/responses') && entry.options.headers['X-Request-ID'] === requestId);
    ...
    return new Response(JSON.stringify({ id: 'response-safe', status: 'completed', usage: { output_tokens_details: { reasoning_tokens: 3 } } }), { status: 200 });
  };
